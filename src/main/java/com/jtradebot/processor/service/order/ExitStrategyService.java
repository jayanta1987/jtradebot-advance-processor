...
public class ExitStrategyService {
    ...
    /**
     * 🔥 NEW: Check if entry should be blocked due to recent STOPLOSS_HIT exit
     * Returns true if entry should be blocked, false if entry is allowed
     */
    ...
    public void exitOrder(String orderId, ExitReasonEnum exitReason, Double exitPrice, Double exitIndexPrice, Date exitTime) {
        ...
        // 🔥 NEW: Update in-memory exit tracking for STOPLOSS_HIT prevention
        updateExitTracking(exitReason, exitTime);
     ...
     * 🔥 NEW: Update in-memory exit tracking for STOPLOSS_HIT prevention
     */
    private void updateExitTracking(ExitReasonEnum exitReason, Date exitTime) {
