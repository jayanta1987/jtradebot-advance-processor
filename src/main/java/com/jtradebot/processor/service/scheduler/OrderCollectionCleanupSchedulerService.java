...
public class OrderCollectionCleanupSchedulerService {
    ...
    /**
     * Manual cleanup method that can be called via API endpoint or other services
     * This method returns the cleanup result for external callers
     * 
     * @return OrderCleanupResult containing cleanup status and details
     */
    public OrderCleanupResult performManualCleanup() {
        OrderCleanupResult result = new OrderCleanupResult();
        
        try {
            log.info("🧹 MANUAL ORDER CLEANUP - Starting manual order cleanup");

            List<JtradeOrder> allOrders = jtradeOrderRepository.findAll();
            result.setTotalOrdersBeforeCleanup(allOrders.size());

            if (allOrders.isEmpty()) {
                result.setSuccess(true);
                result.setMessage("No orders found in collection");
                result.setCleanupPerformed(false);
                return result;
            }
            ...
            if (hasOldOrders) {
                ...
                result.setOrdersDeleted(allOrders.size());
                result.setMessage("Found old orders and cleared " + allOrders.size() + " orders from collection");
            } else {
                result.setCleanupPerformed(false);
                result.setOrdersDeleted(0);
                result.setMessage("All orders are from today, no cleanup needed");
            }

            result.setSuccess(true);
            log.info("✅ MANUAL ORDER CLEANUP - Completed: {}", result.getMessage());

        } catch (Exception e) {
            log.error("❌ MANUAL ORDER CLEANUP - Error during cleanup: {}", e.getMessage(), e);
    ...
    /**
     * Result class for manual cleanup operations
     */
    public static class OrderCleanupResult {
        private boolean success;
        private String message;
        private int totalOrdersBeforeCleanup;
        ...
        public int getTotalOrdersBeforeCleanup() {
            return totalOrdersBeforeCleanup;
        }

        public void setTotalOrdersBeforeCleanup(int totalOrdersBeforeCleanup) {
            this.totalOrdersBeforeCleanup = totalOrdersBeforeCleanup;
        }
        ...
        public int getOrdersDeleted() {
            return ordersDeleted;
        }

        public void setOrdersDeleted(int ordersDeleted) {
            this.ordersDeleted = ordersDeleted;
        }
    }
}
