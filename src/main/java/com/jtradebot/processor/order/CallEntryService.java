...
@Slf4j
@Service
public class CallEntryService extends BaseEntryService {

    public CallEntryService(TickDataManager tickDataManager,
    ...
    protected boolean isValidData(OrderInfo orderInfo) {
        CallData callData = orderInfo.getCpDetails().getCallData();
        return callData.isValidCall();
    }

    @Override
    protected boolean isOppositeScoreRising(CpDetails cpDetails) {
        PutData putData = cpDetails.getPutData();
        return putData.getAvgScore() > 6;
    }
    ...
    protected void setStopLossTarget(CpDetails cpDetails, TradeOrder tradeOrder, EntryReason entryReason) {
        ...
        Set<Support> supportSet = callData.getSupportMap().get(candleTimeFrame);
        Set<Resistance> resistanceSet = callData.getResistanceMap().get(candleTimeFrame);
        ...
        // Check for breakout scenario (no clear resistance levels)
        boolean isBreakoutScenario = isCallBreakoutScenario(cpDetails, minResistanceValue, tradeOrder.getEntryIndexPrice());
        
        if (isBreakoutScenario) {
            // Use breakout strategy for CALL
            handleCallBreakoutScenario(tradeOrder, cpDetails);
        } else {
            ...
            int adjustedTarget = calculateCpAdjustedTarget(minResistanceValue, tradeOrder.getEntryIndexPrice(), cpAbs, cpDelta, true);
            
            tradeOrder.setIndexStopLoss(maxSupportValue);
            tradeOrder.setIndexTarget(adjustedTarget);
        }
    }
    
    /**
     * Detects breakout scenario for CALL orders
     */
    private boolean isCallBreakoutScenario(CpDetails cpDetails, int resistanceValue, double entryPrice) {
        // Check if resistance is very close to entry (indicating no clear resistance)
        double resistanceDistance = Math.abs(resistanceValue - entryPrice);
        boolean noClearResistance = resistanceDistance < 3.0;
        
        // Check for strong upward momentum
        double cpDelta = cpDetails.getCpDelta();
        boolean strongUpwardMomentum = cpDelta > 2.0;
        ...
        boolean futureSignalAligned = cpDetails.isFutureSignal() && cpDetails.getFutureTrend() == TrendEnum.UP;
        
        return noClearResistance && (strongUpwardMomentum || strongCP || futureSignalAligned);
    }
    
    /**
     * Handles breakout scenario for CALL orders
     */
    private void handleCallBreakoutScenario(TradeOrder tradeOrder, CpDetails cpDetails) {
        double entryPrice = tradeOrder.getEntryIndexPrice();
        double cpAbs = cpDetails.getCpAbs();
        double cpDelta = cpDetails.getCpDelta();
        
        // Calculate breakout targets
        double breakoutDistance = calculateCallBreakoutDistance(cpAbs, cpDelta);
        
        // Set aggressive upward targets
        ...
        // Set tight stop loss below entry
        ...
        String breakoutComment = String.format(
                "CALL BREAKOUT: CP=%.2f, Delta=%.2f, Target Distance=%.1f points",
                cpAbs, cpDelta, breakoutDistance
        );
        tradeOrder.addComment(breakoutComment);
        
        log.info("CALL Breakout strategy applied: Entry={}, Target={}, StopLoss={}", 
                entryPrice, breakoutTarget, breakoutStopLoss);
    }
    
    /**
     * Calculates breakout distance for CALL orders
     */
    private double calculateCallBreakoutDistance(double cpAbs, double cpDelta) {
        ...
        // Adjust based on upward momentum
        if (cpDelta > 3.0) {
            baseDistance += 10.0; // High upward momentum
        } else if (cpDelta > 2.0) {
            baseDistance += 5.0; // Moderate upward momentum
        } else if (cpDelta < 0.5) {
    ...
    /**
     * Calculates CP-adjusted target for CALL orders
     */
